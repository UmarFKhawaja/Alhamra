...
Entities such as treatments and treatment categories require a URL-safe slug for routing and reference. Asking users to manually type a slug alongside every name is error-prone, while silently overwriting a manually chosen slug on every name change is frustrating.

A treatment category slug also serves as a stable identifier. A name change should update the slug automatically only when the slug has not been intentionally set by the user.
...
The `slugify` function is a shared utility at `app/lib/utils/slugify.ts`:
...
Server-side upserts still apply `slugify` as a fallback when the submitted slug is empty, ensuring that a blank slug is never persisted.
